...
function copyStartMenuFolders(appDir, appId, appVers, startMenuFoldersDirectory) {
  ...
  printFormattedDebug(`Found startMenuFolders dir in ${appId}`);
  mkdirp(startMenuFoldersDirectory, FOLDER_MODE);

  fs.readdirSync(pluginFoldersLocation, {withFileTypes: true}).filter(statObj => statObj.isFile())
    .forEach((statObj) => {
      const filename = statObj.name;
      const filepath = path.resolve(pluginFoldersLocation, filename);
      const destPath = path.resolve(startMenuFoldersDirectory, appId + '.json');
      try {
        const folderData = JSON.parse(fs.readFileSync(filepath, 'utf8'));
        // Stamp metadata
...
function registerPlugin(pluginPath, pluginDefinition, pluginPointerDirectory, pluginActionsLocation, pluginRecognizersLocation, runtimeDirectory, startMenuFoldersLocation) {
