...
const startMenuFoldersStorage = path.join(config.instanceDir, 'ZLUX/pluginStorage/org.zowe.zlux.ivydesktop/ui/startMenu', 'folders');
initUtils.mkdirp(startMenuFoldersStorage, initUtils.FOLDER_MODE);
      ...
      manifest.appfwPlugins.forEach(function (manifestPluginRef) {
        ...
        if (pluginDefinition && initUtils.fileExists(pluginDefinition)) {
          ...
          if (enabled) {
            ...
            initUtils.registerPlugin(fullPath, pluginDefinitionJson, config.pluginsDir, actionsPluginStorage, recognizersPluginStorage, RUNTIME_DIRECTORY, startMenuFoldersStorage);
          } else {
...
// Fallback: scan already-registered plugin references to copy start menu folders
// for plugins not discovered through component iteration (e.g. dev environments)
if (initUtils.directoryExists(config.pluginsDir)) {
  const pluginRefs = fs.readdirSync(config.pluginsDir).filter(f => f.endsWith('.json'));
  pluginRefs.forEach(refFile => {
    try {
      const refContent = JSON.parse(fs.readFileSync(path.join(config.pluginsDir, refFile), 'utf8'));
      let pluginLocation = refContent.pluginLocation;
      if (refContent.relativeTo === '$ZWE_zowe_runtimeDirectory' && RUNTIME_DIRECTORY) {
        pluginLocation = path.join(RUNTIME_DIRECTORY, pluginLocation);
      }
      if (pluginLocation && initUtils.directoryExists(pluginLocation)) {
        const pluginDefPath = path.join(pluginLocation, 'pluginDefinition.json');
        if (initUtils.fileExists(pluginDefPath)) {
          const pluginDef = JSON.parse(fs.readFileSync(pluginDefPath, 'utf8'));
          initUtils.copyStartMenuFolders(pluginLocation, pluginDef.identifier, pluginDef.pluginVersion, startMenuFoldersStorage);
        }
      }
    } catch (e) {
      initUtils.printFormattedDebug(`Could not process plugin ref ${refFile} for start menu folders: ${e.message}`);
    }
  });
}
