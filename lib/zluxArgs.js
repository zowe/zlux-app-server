...
if(process.env.overrideFileConfig !== "false"){
  ...
  if (Object.keys(envConfig).length > 0) {
    ...
    configJSON.components['app-server'] = mergeUtils.deepAssign(configJSON.components['app-server'], envConfig);  }
  if (userInput.D) {
    if (cluster.isPrimary) {
    ...
    configJSON.components['app-server'] = mergeUtils.deepAssign(configJSON.components['app-server'], userInput.D);
  }
} else {
