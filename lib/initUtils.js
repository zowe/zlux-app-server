...
const PLUGIN_IDENTIFIER_PATTERN = /^[A-Za-z0-9_.-]+$/;

function isValidPluginIdentifier(identifier) {
  return typeof identifier === 'string' &&
      identifier.length > 0 &&
      PLUGIN_IDENTIFIER_PATTERN.test(identifier) &&
      !identifier.includes('..');
}
module.exports.isValidPluginIdentifier = isValidPluginIdentifier;
...
function deregisterPlugin(pluginDefinition, pluginPointerDirectory, actionsDirectories, startMenuFoldersLocation) {
  if (!pluginDefinition || !isValidPluginIdentifier(pluginDefinition.identifier)) {
    printFormattedError(`Refusing to deregister plugin: invalid plugin identifier '${pluginDefinition ? pluginDefinition.identifier : pluginDefinition}'`);
    return false;
  }

  const filePath = path.resolve(pluginPointerDirectory, `${pluginDefinition.identifier}.json`);
  let removed = false;
...
function registerPlugin(pluginPath, pluginDefinition, pluginPointerDirectory, pluginActionsLocations, pluginRecognizersLocations, runtimeDirectory, startMenuFoldersLocation) {
  if (!pluginDefinition || !isValidPluginIdentifier(pluginDefinition.identifier)) {
    printFormattedError(`Refusing to register plugin: invalid plugin identifier '${pluginDefinition ? pluginDefinition.identifier : pluginDefinition}'`);
    return;
  }

  const pointerPath = path.resolve(pluginPointerDirectory, `${pluginDefinition.identifier}.json`);
