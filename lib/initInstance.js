...
let destination;
if (process.env.INSTANCE_DIR && process.env.INSTANCE_DIR != '""') {
  destination = path.join(process.env.INSTANCE_DIR, 'app-fw');
} else {
  destination = process.env.USERPROFILE
    ? path.join(process.env.USERPROFILE, '.zowe', 'app-fw')
    : path.join(process.env.HOME, '.zowe', 'app-fw');
}
